#!/usr/bin/env python3
"""
FVGC v1.9.1 Signal Detector

v1.9.1: BOS requires entry candle to be FIRST to break structure. Re-tap
required for second entry from same FVG.
...
def detect_fvg(candles: pd.DataFrame, idx: int) -> Optional[Dict]:
    ...
    if c1['timestamp_ny'].time() < FVG_START_TIME:
        return None
...
def compute_swing_points(candles: pd.DataFrame, n: int = SWING_LOOKBACK) -> pd.DataFrame:
    ...
    for i in range(n, len(candles) - n):
        ...
        is_low = all(
            ...
            for j in range(1, n + 1))
        if is_high:
            swings.append({'idx': i, 'type': 'high', 'price': h, 'confirmed_at_idx': i + n})
        if is_low:
            swings.append({'idx': i, 'type': 'low', 'price': l, 'confirmed_at_idx': i + n})
    if swings:
...
def _classify_variant(
    ...
    """v1.9.1 §3.3: BOS requires entry candle to be FIRST to close through swing.
    Swing touch invalidation checks the swing relevant to the direction."""
    if direction == 'long':
        swing_break = current_swing_at(swings_df, idx, 'high')
        entry_breaks = swing_break is not None and candle['close'] > swing_break['price']
        invalidation_swing_type = 'low'
    else:
        swing_break = current_swing_at(swings_df, idx, 'low')
        entry_breaks = swing_break is not None and candle['close'] < swing_break['price']
        invalidation_swing_type = 'high'

    touched_invalidation = _window_touched_swing(
        tap_idx, idx, candles, swings_df, invalidation_swing_type)

    if entry_breaks:
        if _is_first_break(tap_idx, idx, candles, swing_break, direction):
            return 'bos'
        return None
...
def _try_entry_cascading(
    ...
    """v1.9.1: Cascading tap + re-tap rule + opposing FVG check."""
    for fvg in active_fvgs:
        if not fvg['active']:
            continue
        if fvg.get('needs_retap'):
...
def log_signals(signals: List[Dict]):
    ...
    pd.DataFrame(rows).to_csv(TRADES_LOG_PATH, index=False)
    print(f"Logged {len(rows)} signals -> {TRADES_LOG_PATH}")
...
def main():
    print("=" * 60)
    print("FVGC v1.9.1 Signal Detector")
    print("=" * 60)
