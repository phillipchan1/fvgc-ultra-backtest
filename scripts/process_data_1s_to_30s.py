...
def resample_chunk_to_30s(df):
    ...
    For each 30-second timestamp window, selects the contract with highest volume
    and only aggregates OHLCV from that contract. This prevents mixing contracts
    like NQZ5 and NQH6 in the same bar.
    """
    if df.empty:
        return df
    
    # Round timestamps to nearest 30 seconds
    df = df.copy()
    df['ts_event_rounded'] = df['ts_event'].dt.floor('30s')
    
    # Process each 30-second bucket
    all_bars = []
    
    for ts_bucket, group in df.groupby('ts_event_rounded'):
        # Find contract with highest volume in this bucket
        volume_by_contract = group.groupby('symbol')['volume'].sum()
        
        if len(volume_by_contract) == 0:
            continue
        
        active_contract = volume_by_contract.idxmax()
        active_contract_data = group[group['symbol'] == active_contract].copy()
        
        if len(active_contract_data) == 0:
            continue
        
        # Optional: Outlier filtering (remove ticks >100 points from median)
        median_close = active_contract_data['close'].median()
        outlier_threshold = 100.0
        active_contract_data = active_contract_data[
            abs(active_contract_data['close'] - median_close) <= outlier_threshold
        ]
        
        if len(active_contract_data) == 0:
            continue
        
        # Aggregate OHLCV for the active contract only
        bar = {
            'timestamp': ts_bucket,
            'open': active_contract_data['open'].iloc[0],
            'high': active_contract_data['high'].max(),
            'low': active_contract_data['low'].min(),
            'close': active_contract_data['close'].iloc[-1],
            'volume': active_contract_data['volume'].sum(),
            'symbol': active_contract
        }
        
        # Only add if we have valid OHLCV
        if pd.notna(bar['open']) and pd.notna(bar['high']) and pd.notna(bar['low']) and pd.notna(bar['close']):
            all_bars.append(bar)
