#!/usr/bin/env python3
"""Indicator-only baseline: full consolidated 5m history, no narrative filters."""
...
DATA_PATH = Path('data/consolidated/nq-front-month.ohlcv-5m.csv')
LOGS_DIR = Path('logs')


def main():
    print("=" * 60)
    print(f"FVGC v{__version__} — Indicator baseline (5m, full history)")
    print("=" * 60)
    ...
    print("\n--- Full-history summary (5m) ---")
    print_summary(stats)

    out_trades = LOGS_DIR / 'baseline_5m_trades.csv'
    out_fvgs = LOGS_DIR / 'baseline_5m_fvgs.csv'
