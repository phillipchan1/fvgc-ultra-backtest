#!/usr/bin/env python3
"""Indicator-only baseline: full consolidated 3m history, no narrative filters."""
...
DATA_PATH = Path('data/consolidated/nq-front-month.ohlcv-3m.csv')
LOGS_DIR = Path('logs')


def main():
    print("=" * 60)
    print(f"FVGC v{__version__} — Indicator baseline (3m, full history)")
    print("=" * 60)
    ...
    print("\n--- Full-history summary (3m) ---")
    print_summary(stats)

    out_trades = LOGS_DIR / 'baseline_3m_trades.csv'
    out_fvgs = LOGS_DIR / 'baseline_3m_fvgs.csv'
