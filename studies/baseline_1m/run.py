#!/usr/bin/env python3
"""Indicator-only baseline: full consolidated 1m history, no narrative filters."""
...
DATA_PATH = Path('data/consolidated/nq-front-month.ohlcv-1m.csv')
LOGS_DIR = Path('logs')


def main():
    print("=" * 60)
    print(f"FVGC v{__version__} — Indicator baseline (1m, full history)")
    print("=" * 60)
    ...
    print("\n--- Full-history summary (1m) ---")
    print_summary(stats)

    out_trades = LOGS_DIR / 'baseline_1m_trades.csv'
    out_fvgs = LOGS_DIR / 'baseline_1m_fvgs.csv'
